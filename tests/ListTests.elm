module ListTests exposing (testListOfN, tests, toDictList)
...
testListOfN n =
    ...
    describe (toString n ++ " elements")
        ...
        , describe "sortWith"
            [ test "sorted" <| \() -> Expect.equal xsNeg (sortWith (\b a -> compare a b) xsNeg)
            , test "unsorted" <| \() -> Expect.equal xsNeg (sortWith (\b a -> compare a b) xsOpp)
            ]
        ]
